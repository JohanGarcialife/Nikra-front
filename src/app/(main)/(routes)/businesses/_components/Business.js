...
export default function Business(props) {
  ...
  useEffect(() => {
    ...
    if (business.imagen) {
      apiClient
        .get(`/api/upload/associate/${business.imagen}`, {
          responseType: "blob",
        })
        .then((response) => {
  ...
  return (
    <div className="relative w-full bg-white shadow-xl my-5 border-t-4 border-b-4 border-primary border-x-0 flex flex-col items-center overflow-hidden rounded-lg transition duration-200 ease-in-out hover:-translate-y-0.5">
      ...
      <div className="text-center bg-white py-1.5 px-2.5 text-primary text-sm font-bold">
        {/* .titleBusiness: text-center, py-1.5 (6px), px-2.5 (10px), color custom, text-sm (small) */}
        {business.nombre}
      </div>

      <div className="text-center py-1.5 px-2.5 text-primary text-xs">
