...
export const MobileSelect = memo(({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      document.removeEventListener('touchmove', preventTouch, { passive: false, capture: true });
      document.removeEventListener('touchend', preventTouch, { passive: false, capture: true });
    };
  }, [open, isMobile]);
  ...
  useEffect(() => {
    ...
    listContainer.addEventListener('touchend', handleTouchEnd, { passive: false });
    ...
    const drawerContent = (
      ...
      >
        ...
        >
          ...
          >
            <div 
              className="h-[5px] w-[60px] rounded-full bg-[#133D74]/40 group-active/drag:bg-[#133D74]/60 transition-all duration-200"
            />
          </div>
          ...
          {label && (
            <div className="px-4 pb-2">
              <h3 className="text-lg font-semibold text-[#133D74]">{label}</h3>
            </div>
          )}
            ...
            <div className="sticky top-0 bg-white z-10 px-4 pb-4">
              <div className="relative">
                <input
                  ...
                  autoFocus={false}
                  className="w-full p-3 border-2 border-[#133D74]/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#3E6FA7] focus:border-transparent transition-all bg-white/80 text-[#36454F]"
                />
              </div>
            </div>
          )*/}
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className={`${isSelected ? 'font-semibold text-[#133D74]' : 'text-[#36454F]'} flex-1`}>
                          {opt.name}
                        </span>
                        ...
                        {isSelected && (
                          <Check
                            size={20}
                            className="text-[#133D74] ml-2 flex-shrink-0"
                          />
                        )}
                      </div>
                    </li>
  ...
  return (
    <MotionUl
      ...
      className="absolute z-50 w-full bg-white border-2 border-[#133D74] rounded-lg shadow-xl mt-1 max-h-60 overflow-y-auto"
      style={{
        WebkitOverflowScrolling: 'touch'
      }}
    >
      ...
      {searchable && (
        <div className="sticky top-0 bg-white z-10 p-2 border-b border-gray-200">
          <input
            ...
            placeholder={`Buscar...`}
            autoFocus
            className="w-full p-2 border border-[#133D74]/30 rounded focus:outline-none focus:ring-2 focus:ring-[#3E6FA7] bg-white/80 text-[#36454F] text-sm"
          />
        </div>
      )}
      ...
      {filteredOptions.length > 0 ? (
        filteredOptions.map((opt) => {
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <span className={`${isSelected ? 'font-semibold text-[#133D74]' : 'text-[#36454F]'}`}>
                  {opt.name}
                </span>
